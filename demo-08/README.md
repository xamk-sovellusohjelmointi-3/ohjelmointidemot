...
Androidilla kirjasto lisää sovellukseen automaattisesti oikeudet `ACCESS_COARSE_LOCATION` (likimääräinen sijainti) ja `ACCESS_FINE_LOCATION` (tarkka sijainti). iOS:llä lupakyselyssä näytettävä selitysteksti annetaan `app.json`-tiedoston config pluginissa. `npx expo install` ei lisää pluginia `app.json`-tiedostoon, joten lisää se `plugins`-kenttään itse:
...
| `android.accuracy` | `'fine'`, `'coarse'` tai `'none'`: antoiko käyttäjä tarkan vai likimääräisen sijainnin |
| `ios.scope` | `'whenInUse'`, `'always'` tai `'none'` |
| `ios.accuracy` | `'full'` tai `'reduced'`: antoiko käyttäjä tarkan vai likimääräisen sijainnin |
...
| `coords.accuracy` | Epätarkkuussäde metreinä. Voi olla `null`. |
...
**Fyysinen iPhone (Linux ja Windows).** iOS-simulaattori toimii vain macOS:llä, joten Linuxilla ja Windowsilla iOS-versiota testataan omalla iPhonella. Expo Go on App Storessa ilmainen, eikä se vaadi Apple Developer Program -jäsenyyttä. iOS:n Expo Go vaatii 3.9.2026 alkaen kuitenkin kirjautumisen samalle ilmaiselle [Expo-tilille](https://expo.dev/signup) sekä terminaalissa että sovelluksessa:
